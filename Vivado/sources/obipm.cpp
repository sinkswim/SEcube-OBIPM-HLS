...
// Pure Comb module, code for handling inout port not present...
    ...
    while(true){
        ...
        if(irq.read() == '1'){
            for(int i = 0; i < NUM_IP; i++){
                if(intr_ip_v[i] == '1'){          // NOTE: on g++ linux compiler "" gave problems.... check on stratus
...
// SC_CTHREAD, reset signal ?
// NOTE: the interrupt controller at EACH clock cycle:
//  - if int asserted, check in the same clock cycle which has the high priority
//    once is found, then serve it and wait until ack signal arrives
//  - when ack arrives, deassert interrupt request and if there are still interrupt pending serve it(
//    remember that high priority must be served first always)
//
